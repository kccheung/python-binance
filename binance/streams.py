...
class UserDataWebsocketSBE(ReconnectingWebsocket):
    ...
    def __init__(self, client: AsyncClient, loop, url: str, path: Optional[str] = None, prefix: str = 'ws-api/v3?returnRateLimits=false&responseFormat=sbe&sbeSchemaId=3&sbeSchemaVersion=5', exit_coro=None,
...
class BinanceSocketManager:
    ...
    def _get_account_sbe_socket(self, option: Optional[int] = None, prefix: str = 'ws-api/v3?returnRateLimits=false&responseFormat=sbe&sbeSchemaId=3&sbeSchemaVersion=5', q: Optional[asyncio.Queue] = None):
        conn_id = f'{BinanceSocketType.ACCOUNT}{option if option in [0, 1] else self._default_option}'
        if conn_id not in self._conns:
